        ...
        $('.mdui-col-xs-12 i.mdui-icon').each(function () {
            if (
                ...
                $(this)
                    ...
                    .css(
                        'background',
                        'url(' + href + thumb + ')  no-repeat center top'
                    )
            }
        })
    }
...
// 分享链接
document.getElementById('share-dialog').addEventListener('open.mdui.dialog', function () {
...
$('#newfolder').on('click', function () {
    mdui.prompt(
        ...
        function (value) {
            ...
            var query =
                'foldername=' + value + '&uploadurl=' + window.location.href
            ...
            httpRequest.onreadystatechange = function () {
                if (httpRequest.readyState == 4 && httpRequest.status == 200) {
                    ...
                    if (item.id) {
                        var $folder_domstr = $(
                            '<li class="mdui-list-item mdui-ripple filter" data-sort data-sort-name="' +
                            item.name +
                            '" data-sort-date="' +
                            ...
                            '" id="' +
                            item.id +
                            '"><label class="mdui-checkbox"><input type="checkbox" value="' +
                            item.id +
                            '" name="itemid" onclick="onClickHander()"><i class="mdui-checkbox-icon"></i></label><a href="' +
                            window.location.href +
                            item.name +
                            '"><div class="mdui-col-xs-12 mdui-col-sm-7 mdui-text-truncate"><i class="mdui-icon material-icons">folder_open</i><span>' +
                            item.name +
                            '</span></div><div class="mdui-col-sm-3 mdui-text-right">' +
                            item.lastModifiedDateTime.replace(
                                /[a-zA-Z]/g,
                                ' '
                            ) +
                            '</div><div class="mdui-col-sm-2 mdui-text-right">' +
                            item.size +
                            '</div></a></li>'
                        )
        ...
        function (value) {
            ...
            httpRequest.onreadystatechange = function () {
                ...
                if (httpRequest.readyState == 4 && httpRequest.status == 200) {
                    ...
                    item_dom.getElementsByClassName(
                        'loading-gif'
                    )[0].style.display = 'none'
                    item_dom
                        .getElementsByTagName('a')[0]
                        .setAttribute('href', a_href)
    ...
    mdui.confirm(
        ...
        function () {
            for (var i = 0; i < check_val.length; i++) {
                document
                    .getElementById(check_val[i])
                    .getElementsByClassName('loading-gif')[0].style.display = ''
            }
                ...
                if (httpRequest.readyState == 4 && httpRequest.status == 200) {
                    ...
                    for (var i = 0; i < check_val.length; i++) {
                        if (resp[i]) {
                            ...
                            errormessage = JSON.parse(resp[i])['error'][
                                'message'
                            ]
                        } else {
...
function paste() {
    ...
    if (getCookie('cutitems')) {
        ...
        httpRequest.onreadystatechange = function () {
            ...
            document.cookie =
                'copyitems=; expires=Thu, 01 Jan 1970 00:00:00 GMT'
        }
    }
}
//获取cookie
...
function getListDom(item) {
    ...
    if (item['folder']) {
        $domstr =
            ...
            item.name +
            '</span></div><div class="mdui-col-sm-3 mdui-text-right">' +
            item.lastModifiedDateTime.replace(/[a-zA-Z]/g, ' ') +
            '</div><div class="mdui-col-sm-2 mdui-text-right">' +
            item.size +
            '</div></a></li>'
    } else {
        $domstr =
            '<li class="mdui-list-item file mdui-ripple filter" data-sort data-sort-name="' +
            item.name +
            '" data-sort-date="' +
            item.lastModifiedDateTime +
            '" data-sort-size="' +
            ...
            '" name="itemid" onclick="onClickHander()"><i class="mdui-checkbox-icon"></i></label><a href="' +
            path +
            '" target="_blank"><div class="mdui-col-xs-12 mdui-col-sm-7 mdui-text-truncate"><i class="mdui-icon material-icons">insert_drive_file</i><span>' +
            item.name +
            '</span></div><div class="mdui-col-sm-3 mdui-text-right">' +
            item.lastModifiedDateTime.replace(/[a-zA-Z]/g, ' ') +
            '</div><div class="mdui-col-sm-2 mdui-text-right">' +
